...
async function serveStatic(req: IncomingMessage, res: ServerResponse) {
  const url = requestUrl(req)
  const instrumentPaths = url.pathname.startsWith('/instruments/')
    ? resolveInstrumentFilePaths(
        url.pathname,
        getWorkspaceFromRequest(toWebRequest(req)),
      )
    : []
  const filePaths =
    instrumentPaths.length > 0
      ? instrumentPaths
      : [resolveStaticFilePath(url.pathname)]
  ...
  for (const filePath of filePaths) {
    try {
      ...
      // Try the next candidate, then return 404 below.
    }
  }
...
async function main() {
  ...
  server.listen(port, host, () => {
    console.log(`Guitar ready at http://${host}:${port}`)
  })
}
