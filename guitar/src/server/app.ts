...
import {
  DEFAULT_GUITAR_PACK_ID,
  DEFAULT_GUITAR_PRESET_ID,
  GUITAR_INSTRUMENT_PACKS,
  GUITAR_PRESETS,
  GUITAR_SAMPLE_SETS,
  GUITAR_SETTING_CONTROLS,
  type GuitarAudioSettings,
  type GuitarPresetParameters,
  type GuitarSoundChoice,
  type SongSoundSettings,
  type SongSoundSettingsResponse,
  defaultGuitarAudioSettings,
  guitarPresetById,
} from '../shared/audio'
...
import {
  ensureMutopiaGuitarIndex,
  fetchMutopiaMidi,
  readMutopiaGuitarIndexStatus,
  rebuildMutopiaGuitarIndex,
  searchMutopiaGuitarIndex,
...
function countCourseLessons(course: GuitarCourse): number {
...
function courseHasLesson(course: GuitarCourse, lessonId: string): boolean {
...
async function ensureSeedCourses(dataDir: string) {
  ...
  for (const course of defaultCourses) {
    ...
    const existing = await readJson<GuitarCourse | null>(path, null)
    if (!existing) {
...
async function readCourses(dataDir: string): Promise<GuitarCourse[]> {
  const courses: GuitarCourse[] = []
  // Default-order — read the seeded ids first, then any user-added ones
  const seen = new Set<string>()
  for (const id of defaultCourseIds) {
    const course = await readJson<GuitarCourse | null>(
...
async function readCourse(
  dataDir: string,
  courseId: string,
): Promise<GuitarCourse | null> {
  if (!/^[a-z0-9-]+$/.test(courseId)) return null
  return readJson<GuitarCourse | null>(coursePath(dataDir, courseId), null)
}
...
function isPresetId(value: unknown): value is string {
  return (
    typeof value === 'string' &&
    GUITAR_PRESETS.some((preset) => preset.id === value)
  )
}
...
function sanitizeAudioOverrides(
  value: unknown,
): Partial<GuitarPresetParameters> {
  if (!value || typeof value !== 'object') return {}

  const source = value as Record<string, unknown>
  const overrides: Partial<GuitarPresetParameters> = {}
...
function normalizeAudioSettings(settings: GuitarAudioSettings | null) {
  const defaults = defaultGuitarAudioSettings()
...
function normalizeInstrumentChoice(packId: unknown, instrumentId: unknown) {
  ...
  const pack = GUITAR_INSTRUMENT_PACKS.find(
...
async function readAudioSettings(dataDir: string) {
  const settings = await readJson<GuitarAudioSettings | null>(
...
function sanitizeSoundChoice(value: unknown): GuitarSoundChoice {
  if (!value || typeof value !== 'object') return {}
  const source = value as Record<string, unknown>
  const choice: GuitarSoundChoice = {}
...
function mergeSoundChoice(
  base: GuitarSoundChoice,
  next: GuitarSoundChoice | undefined,
): GuitarSoundChoice {
  if (!next) return base
  return {
...
function hasSoundChoice(choice: GuitarSoundChoice | undefined) {
  return Boolean(
...
async function getSongSoundSettingsResponse(
  ...
  const globalChoice: GuitarSoundChoice = {
...
function summarizeSong(song: GuitarSong): SongSummary {
  return {
    id: song.id,
    title: song.title,
    source: song.source,
...
async function persistImportedMidiSong(
  ...
  const existing = await readJson<GuitarSong | null>(destinationSongPath, null)
...
function sanitizeNote(
  ...
  const note: GuitarSong['notes'][number] = {
  ...
  const fretPosition = objectParam(source.fretPosition)
  const stringIndex = numberParam(fretPosition.stringIndex)
  const fret = numberParam(fretPosition.fret)
  if (
    ...
    note.fretPosition = { stringIndex, fret }
  }
...
function sanitizeSongDocument(
  ...
  const notes = source.notes
    ...
    .filter((note): note is GuitarSong['notes'][number] => Boolean(note))
  ...
  const sourceInfo =
    source.sourceInfo && typeof source.sourceInfo === 'object'
      ? (source.sourceInfo as GuitarSong['sourceInfo'])
      : existing?.sourceInfo
  ...
  return {
    ...
    tempoMap: Array.isArray(source.tempoMap)
      ? (source.tempoMap as GuitarSong['tempoMap'])
      : existing?.tempoMap,
    timeSignatureMap: Array.isArray(source.timeSignatureMap)
      ? (source.timeSignatureMap as GuitarSong['timeSignatureMap'])
...
function sanitizeSongTutorial(value: unknown): GuitarSong['tutorial'] {
  ...
  return {
    ...
    sourceInfo:
      metadata.sourceInfo && typeof metadata.sourceInfo === 'object'
        ? (metadata.sourceInfo as GuitarSong['sourceInfo'])
        : existing.sourceInfo,
    bpm,
    beatsPerBar,
    beatUnit,
    ...
    tempoMap: Array.isArray(metadata.tempoMap)
      ? (metadata.tempoMap as GuitarSong['tempoMap'])
      : existing.tempoMap,
    timeSignatureMap: Array.isArray(metadata.timeSignatureMap)
      ? (metadata.timeSignatureMap as GuitarSong['timeSignatureMap'])
      : existing.timeSignatureMap,
    midiInfo: existing.midiInfo,
    tutorial:
...
async function patchSong(dataDir: string, rawParams: PatchSongParams) {
  ...
  const existing = await readJson<GuitarSong | null>(
...
async function ensureSeedSongs(dataDir: string) {
  ...
  for (const id of retiredDefaultSongIds) {
    ...
    const existing = await readJson<GuitarSong | null>(path, null)
...
app.get('/api/moldable/health', (c) => {
  return c.json({
    appId: process.env.MOLDABLE_APP_ID ?? 'guitar',
    status: 'ok',
  })
})
    ...
    return c.json(
      {
        ...
        error: {
          ...
          message: 'Guitar RPC requires a method string.',
        },
      },
      400,
    )
    ...
    return c.json(
      {
        ...
        error: {
          ...
          message: `Guitar does not expose ${body.method}.`,
        },
      },
      404,
    )
    ...
    return c.json(
      {
        ...
        error: {
          code: 'internal_error',
          message:
            error instanceof Error ? error.message : 'Guitar RPC failed.',
        },
      },
      500,
    )
...
app.get('/api/audio/options', async (c) => {
  const dataDir = getDataDir(c)
  const instrumentPacks = await withInstrumentInstallStates(
    GUITAR_INSTRUMENT_PACKS,
    dataDir,
  )
  return c.json({
    defaultPresetId: DEFAULT_GUITAR_PRESET_ID,
    instrumentPacks: instrumentPacks.toSorted((a, b) => {
      if (a.id === DEFAULT_GUITAR_PACK_ID) return -1
      if (b.id === DEFAULT_GUITAR_PACK_ID) return 1
      return 0
    }),
    sampleSets: GUITAR_SAMPLE_SETS,
    presets: GUITAR_PRESETS,
    controls: GUITAR_SETTING_CONTROLS,
  })
})
...
app.get('/api/audio/settings', async (c) => {
  try {
    ...
    return c.json({
      settings,
      preset: guitarPresetById(settings.presetId),
    })
  } catch (error) {
    return jsonError(
      c,
...
app.patch('/api/audio/settings', async (c) => {
  try {
    ...
    const settings: GuitarAudioSettings = {
...
app.get('/api/song-catalog/mutopia/status', async (c) => {
  try {
    return c.json(await readMutopiaGuitarIndexStatus(getDataDir(c)))
  } catch (error) {
    return jsonError(
      c,
      error instanceof Error
...
app.post('/api/song-catalog/mutopia/rebuild', async (c) => {
  try {
    const index = await rebuildMutopiaGuitarIndex(getDataDir(c))
    return c.json({
...
app.get('/api/song-catalog/search', async (c) => {
  try {
    ...
    const dataDir = getDataDir(c)
    const index = await ensureMutopiaGuitarIndex(dataDir)
    ...
    return c.json({
      ...
      repositoryCommit: index.repositoryCommit,
      generatedAt: index.generatedAt,
      results: searchMutopiaGuitarIndex(index, query, limit).map((entry) => ({
        ...entry,
...
app.delete('/api/songs/:songId', async (c) => {
  try {
    ...
    const existing = await readJson<GuitarSong | null>(filePath, null)
