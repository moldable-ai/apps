...
const installManifestName = '.guitar-pack-install.json'
...
export async function withInstrumentInstallState(
  pack: GuitarInstrumentPack,
  dataDir: string,
) {
  if (pack.download.strategy === 'bundled') {
    return {
      ...pack,
      status: 'installed',
      installedPath: path.join(bundledInstrumentsDir(), pack.id),
    } satisfies GuitarInstrumentPack
  }

  if (pack.download.strategy === 'cdn') {
    return {
      ...pack,
      status: 'installed',
    } satisfies GuitarInstrumentPack
...
export async function installInstrumentPack(
  ...
  const response = await fetch(pack.downloadUrl, {
    headers: {
      'user-agent': 'Moldable Guitar/0.1 instrument installer',
    },
  })
