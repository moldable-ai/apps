...
function indexPath(dataDir: string) {
  return safePath(catalogDir(dataDir), 'mutopia-guitar-index.json')
}
...
function isDirectSoloGuitarEntry(relativePath: string, source: string) {
  ...
  return fields.mutopiainstrument === 'Guitar'
}
...
function entryFromSource(
  ...
  if (!isDirectSoloGuitarEntry(relativePath, source)) return null
...
async function fetchJson<T>(url: string): Promise<T> {
  const response = await fetch(url, {
    headers: {
      ...
      'User-Agent': 'moldable-guitar-song-catalog',
    },
  })
  if (!response.ok) {
...
export async function buildMutopiaGuitarIndex(dataDir: string) {
...
export async function ensureMutopiaGuitarIndex(dataDir: string) {
  ...
  const rebuild = buildMutopiaGuitarIndex(dataDir).finally(() => {
...
export async function rebuildMutopiaGuitarIndex(dataDir: string) {
  const current = rebuilds.get(dataDir)
  if (current) return current
...
export async function readMutopiaGuitarIndexStatus(dataDir: string) {
  const existing = await readJson<MutopiaIndex | null>(indexPath(dataDir), null)
...
export function searchMutopiaGuitarIndex(
  index: MutopiaIndex,
  query: string,
  limit = 40,
) {
