...
import { GUITAR_PRESETS, type GuitarPresetId } from './audio-presets'
import { formatDuration } from './guitar-utils'
...
function isGuitarPresetId(value: string | undefined): value is GuitarPresetId {
...
export function App() {
  ...
  const [presetId, setPresetId] = useState<GuitarPresetId>(
    isGuitarPresetId(cachedAudioSettings?.presetId)
  ...
  const [activePackId, setActivePackId] = useState<string | null>(
    cachedAudioSettings?.instrumentPackId ?? DEFAULT_GUITAR_PACK_ID,
  )
  const [activeInstrumentId, setActiveInstrumentId] = useState<string | null>(
    cachedAudioSettings?.instrumentId ?? DEFAULT_GUITAR_INSTRUMENT_ID,
  )
  ...
  const activePack = useMemo<GuitarInstrumentPack | null>(() => {
    return (
      instrumentPacks.find((pack) => pack.id === activePackId) ??
      instrumentPacks.find((pack) => pack.id === DEFAULT_GUITAR_PACK_ID) ??
  ...
  const persistAudioSettings = useCallback(
    async (settings: Partial<GuitarAudioSettings>) => {
      const res = await fetchWithWorkspace('/api/audio/settings', {
        method: 'PATCH',
  ...
  useEffect(() => {
    ...
    setPresetId(
      isGuitarPresetId(settings?.presetId)
        ? settings.presetId
        : DEFAULT_GUITAR_PRESET_ID,
    )
    setActivePackId(settings?.instrumentPackId ?? DEFAULT_GUITAR_PACK_ID)
    setActiveInstrumentId(
      settings?.instrumentId ?? DEFAULT_GUITAR_INSTRUMENT_ID,
    )
  ...
  const persistSongSoundSettings = useCallback(
    async (
      songId: string,
      settings: {
        suggested?: GuitarSoundChoice | null
        override?: GuitarSoundChoice | null
      },
    ) => {
  ...
  useEffect(() => {
    ...
    if (!settings) return
    if (isGuitarPresetId(settings.presetId)) {
  ...
  const songQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      return (await res.json()) as GuitarSong
    },
  ...
  const songPreviewQueries = useQuery({
    ...
    queryFn: async () => {
      ...
      const previews = await Promise.all(
        ids.map(async (id): Promise<[string, GuitarSong['notes']]> => {
          const res = await fetchWithWorkspace(`/api/songs/${id}`)
          if (!res.ok) return [id, []]
          const song = (await res.json()) as GuitarSong
  ...
  const notePreviewBySong = useMemo(
    () => songPreviewQueries.data ?? new Map<string, GuitarSong['notes']>(),
  ...
  useEffect(() => {
    ...
    if (
      isGuitarPresetId(effective.presetId) &&
    ...
    if (pack.status !== 'installed') {
      if (
        ...
        setSongSoundLoadingLabel(`Installing suggested guitar · ${pack.name}…`)
