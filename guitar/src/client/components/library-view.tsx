...
import { formatDuration, midiToTone } from '../guitar-utils'
...
interface LibraryViewProps {
  ...
  isLoading: boolean
  isError: boolean
  notePreviewBySong: Map<string, GuitarNote[]>
...
function songTone(preview: GuitarNote[] | undefined) {
...
function MiniRoll({ notes, tone }: { notes: GuitarNote[]; tone: string }) {
  if (notes.length === 0) {
    return (
      <div className="text-muted-foreground/70 guitar-serif text-xs italic">
        no notes
      </div>
    )
  }
...
function SongCard({
  ...
  return (
    <div
      ...
      className={cn(
        'guitar-card-tone animate-guitar-card-in group relative isolate flex h-56 flex-col overflow-hidden rounded-2xl text-left',
    ...
    >
      ...
      <div className="relative z-10 flex flex-1 flex-col gap-1 p-5">
        ...
        <h3 className="guitar-serif text-foreground line-clamp-2 pr-8 text-xl font-semibold leading-tight tracking-tight">
          {song.title}
        </h3>
      </div>
  ...
  return (
    ...
    >
      ...
      <div className="bg-background/60 border-border/40 relative z-10 flex flex-col gap-1 border-t px-4 pb-3 pt-2 backdrop-blur-sm">
          ...
          previewSongs.map((song) => {
            ...
            return (
              ...
              >
                ...
                />
                <span className="guitar-serif text-foreground truncate">
                  {song.title}
                </span>
                <span className="text-muted-foreground/70 guitar-mono ml-auto shrink-0 text-[10px]">
        ...
        {remainder > 0 ? (
          <p className="text-muted-foreground/70 guitar-mono mt-0.5 text-[10px]">
            +{remainder} more
          </p>
        ) : null}
      </div>
...
function CoursesTabContent({
  ...
  return (
    <section className="animate-guitar-chrome-in">
      <div className="mb-6">
        <h2 className="guitar-serif text-foreground text-3xl font-semibold tracking-tight">
          Courses
        </h2>
...
function EditableFolderTitle({
  ...
  return (
    <h2
      ...
      className={cn(
        'guitar-serif text-foreground text-3xl font-semibold tracking-tight',
...
export function LibraryView({
  ...
  return (
    <div
      className="animate-guitar-view-back relative flex h-full min-h-0 flex-col overflow-hidden"
    ...
    >
      <div className="guitar-no-scrollbar min-h-0 flex-1 overflow-y-auto">
        <div className="mx-auto w-full max-w-5xl px-6 pb-[calc(var(--chat-safe-padding,0px)+4rem)] pt-10">
          {!openFolder && courseSummaries.length > 0 ? (
            <div className="animate-guitar-chrome-in mb-8 flex items-center gap-1">
              <button
                type="button"
              ...
              >
                ...
                <span
                  className={cn(
                    'guitar-mono ml-0.5 inline-flex h-5 min-w-5 items-center justify-center rounded-full px-1.5 text-[10px] tabular-nums',
                    view === 'courses'
          ...
          {!openFolder && view === 'courses' ? null : openFolder ? (
            <section className="animate-guitar-chrome-in mb-10">
              <button
                type="button"
                onClick={() => {
                  setFilter('')
            ...
            <section className="animate-guitar-chrome-in mb-10 flex items-end justify-between gap-4">
              <div className="min-w-0">
                <h2 className="guitar-serif text-foreground text-3xl font-semibold tracking-tight">
                  Tap a song to play it.
                </h2>
                <p className="text-muted-foreground mt-2 max-w-xl text-sm leading-6">
                  Search public-domain solo-guitar pieces from the Mutopia
                  Project or drop MIDI files here to convert them automatically.
                </p>
              </div>
              <div className="flex shrink-0 items-center gap-2">
                <Button
          ...
          {!openFolder && view === 'courses' ? null : (
            <>
              {showFilter ? (
                <div className="animate-guitar-chrome-in mb-5 flex items-center gap-3">
                  ...
                  <span className="text-muted-foreground guitar-mono shrink-0 text-[11px]">
                    {trimmedFilter
              ...
              {isLoading ? (
                ...
                <div className="border-border/60 rounded-2xl border border-dashed p-10 text-center">
                  <p className="guitar-serif text-foreground text-lg">
                    No songs yet
                  </p>
      ...
      {isDraggingMidi ? (
        <div className="bg-background/72 pointer-events-none absolute inset-0 z-30 flex items-center justify-center backdrop-blur-sm">
          <div className="border-border/70 bg-background/95 text-foreground rounded-2xl border px-6 py-5 text-center shadow-xl">
            <p className="guitar-serif text-lg font-semibold">
              {openFolder
                ? `Drop MIDI into ${openFolder.name}`
                : 'Drop MIDI files'}
            </p>
            <p className="text-muted-foreground mt-1 text-sm">
              Guitar will convert .mid and .midi files into playable songs.
            </p>
          </div>
        </div>
      ) : null}
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle className="guitar-serif text-foreground text-lg">
            ...
            <AlertDialogDescription>
              ...
              {folderPendingDelete && folderPendingDelete.songIds.length > 0
                ? `${folderPendingDelete.songIds.length} ${
                  ...
                  } inside will return to your library — no pieces are deleted.`
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This removes the song from your library. You can re-install
              public-domain guitar pieces from the Mutopia catalog or ask
              Moldable chat to transcribe it again.
            </AlertDialogDescription>
          </AlertDialogHeader>
