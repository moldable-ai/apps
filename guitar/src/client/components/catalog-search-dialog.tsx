...
export function CatalogSearchDialog({
  ...
  const indexState = useMemo(() => {
    ...
    return `${statusQuery.data.entryCount} solo-guitar pieces${ago}`
  }, [statusQuery.data])
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="flex max-h-[min(640px,calc(100vh-var(--chat-safe-padding,0px)-2rem))] w-[min(580px,calc(100vw-2rem))] flex-col gap-0 overflow-hidden p-0 sm:max-w-[580px]">
        <div className="border-border/60 border-b px-5 pb-3 pt-5">
          <div className="flex items-start">
            <div className="min-w-0 pr-8">
              <DialogTitle className="guitar-serif text-foreground text-lg font-semibold tracking-tight">
                Find guitar pieces
              </DialogTitle>
              <DialogDescription className="text-muted-foreground mt-1 text-[12px] leading-5">
                Public-domain solo-guitar pieces from the Mutopia Project.
                Install with one click and they appear in your library
                instantly.
              </DialogDescription>
            </div>
          </div>
          ...
          <div className="relative mt-4">
            ...
            <input
              ...
              placeholder="Search by composer, title, or opus…"
              className={cn(
        ...
        <div className="guitar-scrollbar min-h-0 flex-1 overflow-y-auto">
          {installError ? (
          ...
          {!hasQuery ? (
            <div className="px-5 py-6">
              ...
              <p className="text-muted-foreground/70 mt-6 text-[11px] leading-5">
                Tip: searching is fuzzy — try a guitar composer, then narrow
                with a title or opus number.
              </p>
            </div>
            ...
            <div className="px-5 py-10 text-center">
              <p className="guitar-serif text-foreground text-base">
                Nothing matches “{debouncedQuery}”
              </p>
              <p className="text-muted-foreground mt-1 text-[12px]">
                Try a composer name like “Sor” or “Carcassi”.
              </p>
            </div>
          ) : (
                  ...
                  >
                    <div className="min-w-0 flex-1">
                      <div className="flex items-baseline gap-1.5">
                        <h4 className="guitar-serif text-foreground line-clamp-1 text-[14px] font-semibold leading-tight">
                          {result.title}
                        </h4>
                        {result.opus ? (
                          <span className="text-muted-foreground guitar-mono shrink-0 text-[10px]">
                            {result.opus}
                          </span>
                        ) : null}
                      </div>
