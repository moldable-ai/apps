      ...
      <div className="mx-auto max-w-lg">
        ...
        <div className="mt-12">
          ...
          <div className="space-y-3">
            {prompts.map((prompt) => (
              ...
              >
                <span className="bg-primary/10 text-primary ring-primary/20 flex size-6 shrink-0 items-center justify-center rounded-full text-[10px] font-bold ring-1 ring-inset">
                  {prompt.id}
                </span>
        ...
        <div className="mt-12 flex flex-col items-center justify-center gap-3">
          {['M', 'K'].map((key) => (
            <div
              key={key}
              className="border-border bg-card/50 flex items-center gap-2 rounded-full border px-4 py-1.5 shadow-sm"
            >
              ...
              <span className="text-muted-foreground whitespace-nowrap text-[11px] font-medium">
                {key === 'M' ? 'to summon me' : 'to navigate around'}
              </span>
            </div>
          ))}
        </div>
