...
export async function upsertWorkspaceSecret(
  ...
  const dir = await mkdtemp(join(tmpdir(), 'moldable-artifacts-secret-'))
...
export async function invokeAivaultJson<T>(
  ...
  appendAivaultRequestArgs(args, request)
...
export function appendAivaultRequestArgs(
  args: string[],
  request: AivaultRequest,
) {
  ...
  if (request.bodyFilePath) {
    args.push('--body-file-path', request.bodyFilePath)
  ...
  for (const [name, path] of Object.entries(request.multipartFiles ?? {})) {
    args.push('--multipart-file', `${name}=${path}`)
  }
}
