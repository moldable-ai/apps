...
export function sourceFileExtensionForName(name: string): string | null {
  const extension = extname(name).replace(/^\./, '').toLowerCase()
  return /^[a-z0-9]{1,8}$/.test(extension) ? extension : null
}

export function sourceFileExtensionForMime(mimeType: string): string | null {
...
export function sourceImageMimeTypeForName(name: string): string | null {
  switch (sourceFileExtensionForName(name)) {
    case 'png':
      return 'image/png'
    case 'jpg':
    case 'jpeg':
...
export function isSupportedSourceImageName(name: string): boolean {
  const extension = sourceFileExtensionForName(name)
  return Boolean(
    extension &&
...
export function supportedSourceImageError(
  ...
  return new UnsupportedSourceImageError(
    `${subject} is not a supported source image. Images supports ${SOURCE_IMAGE_SUPPORT_LABEL} images for imports.`,
  )
}
...
export async function normalizeUploadedSourceImage({
  ...
  if (isDirectSourceImageMimeType(resolvedMimeType)) {
    return {
      buffer,
      extension:
        sourceFileExtensionForMime(resolvedMimeType) ??
        sourceFileExtensionForName(originalName) ??
        'img',
...
function resolveSourceMimeType(name: string, mimeType?: string): string | null {
  ...
  const fromName = sourceImageMimeTypeForName(name)
