...
export function isPlaidAivaultSystemError(error: unknown): boolean {
  const message =
    error instanceof Error
      ...
      : typeof error === 'string'
        ? error
        : ''
  return /serde error: EOF while parsing a value|thin client daemon attempt failed|Too many open files|Broken pipe/i.test(
    message,
  )
}

export function plaidSyncErrorMessage(error: unknown): string {
  const message = safePlaidErrorMessage(error)
  if (!isPlaidAivaultSystemError(message)) return message
  return [
    'aivault could not read stored Plaid credentials for sync.',
    'Restart aivault and retry; if it persists, reconnect the affected Plaid items.',
    `Original: ${message}`,
  ].join(' ')
}
