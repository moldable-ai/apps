    ...
    if (!url.pathname.includes('.')) {
      try {
        const indexContent = await fs.readFile(
          resolveStaticFilePath('/index.html'),
        )
        res.statusCode = 200
        res.setHeader('Content-Type', 'text/html; charset=utf-8')
        ...
        // Fall through to the standard 404.
      }
    }
    res.statusCode = 404
    res.end('Not found')
...
async function handleClientRequest(
  ...
  const url = requestUrl(req)
  if (shouldRewriteClientRequestPath(url.pathname)) {
    req.url = '/'
  }
...
export function shouldRewriteClientRequestPath(pathname: string) {
...
async function main() {
  ...
  server.listen(port, host, () => {
    console.log(`Money ready at http://${host}:${port}`)
  })
}
