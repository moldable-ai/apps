...
function logGenerateJsonFailure({
  ...
  try {
    console.warn(
      '[money:llm-generate-json-failure]',
      JSON.stringify(record, null, 2),
    )
  } catch {
    console.warn('[money:llm-generate-json-failure]', response.status)
  }

  void appendGenerateJsonFailureDiagnostic(record).catch((error) => {
    console.warn('Failed to write Money LLM diagnostic:', error)
  })
}
...
export async function generateJson<T = unknown>(body: Record<string, unknown>) {
  ...
  if (!res.ok || parseError) {
    ...
    const detail = data.error
      ? `: ${data.error}`
      : responseText
        ? `: ${previewText(responseText, 300)}`
        : ''
    throw new Error(
      `LLM JSON generation failed with status ${res.status}${detail}`,
    )
  }

  return {
    json: data.json as T,
    model: data.model ?? String(body.model ?? 'unknown'),
    usage: data.usage,
  }
}
