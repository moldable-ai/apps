...
describe('money routes', () => {
        ...
        transactions: [
          {
            ...
            source: 'plaid',
            name: 'Cross-Border Transfer Merchant',
          ...
          },
          {
            ...
            source: 'plaid',
            name: 'Transfer Provider Transfer Fee',
            merchantName: 'Transfer Provider',
            amount: 42,
            direction: 'expense',
            category: ['Bank Fees'],
      ...
      body: JSON.stringify({
        ...
        transactions: [
          ...
          },
          {
            ...
            source: 'plaid',
            name: 'Transfer Provider transfer to unlinked Canadian bank',
            amount: 2500,
            direction: 'expense',
            category: ['Transfer'],
            date: '2026-04-22',
  ...
  it('persists attention settings and only surfaces opt-in budget Today items', async () => {
    await app.request('/api/dev/seed', { method: 'POST' })
    ...
    expect(
      today.items.some((item) =>
        item.id.startsWith('money-large-transaction-'),
      ),
    ).toBe(false)
    expect(today.items).toEqual(
      expect.arrayContaining([
  ...
  it('surfaces due recurring obligations in Today', async () => {
    vi.useFakeTimers()
    ...
    expect(today.items).toEqual([
      expect.objectContaining({
        id: 'money-due-recurringobligation-rent',
        kind: 'attention',
        title: 'Rent due Thursday',
        subtitle: '$2,100 · Wells Fargo',
      }),
    ])
  })

  it('skips already-overdue obligations and softens shouty names in Today', async () => {
    vi.useFakeTimers()
    vi.setSystemTime(new Date('2026-06-08T12:00:00.000Z'))
        ...
        transactions: [
          {
            ...
            merchantName: 'MORTGAGE SERVICER',
            amount: 1161,
            direction: 'expense',
            category: ['Mortgage'],
            date: '2026-05-02',
        ...
        extensions: [
          {
            ...
            values: {
              ...
              cadence: 'monthly',
              monthlyAmount: 1161,
              // Past due date: a stale-after-payment obligation must not surface.
    ...
    // Overdue mortgage is dropped; the upcoming utility bill surfaces with a
    // title-cased name and a non-redundant subtitle.
    expect(today.items).toEqual([
      expect.objectContaining({
        id: 'money-due-recurringobligation-utilities',
        kind: 'attention',
        title: 'Acme Utilities due tomorrow',
        subtitle: '$140',
      }),
    ])
    expect(today.items.some((item) => /mortgage/i.test(item.title))).toBe(false)
  })
  ...
  it('treats aivault Plaid credential read failures as one sync-wide blocker', async () => {
    const fakeAivaultPath = path.join(dataDir, 'fake-aivault-serde-sync.mjs')
    const fakeAivaultLogPath = path.join(
      dataDir,
      'fake-aivault-serde-sync.jsonl',
    )
    await fs.writeFile(
      fakeAivaultPath,
      [
        ...
        'const args = process.argv.slice(2)',
        'const capability = args.find((arg) => arg.startsWith("plaid/"))',
        `fs.appendFileSync(${JSON.stringify(fakeAivaultLogPath)}, JSON.stringify({ capability }) + "\\n")`,
        'process.stderr.write("serde error: EOF while parsing a value at line 1 column 0 (thin client daemon attempt failed first: serde error: EOF while parsing a value at line 1 column 0)")',
        'process.exit(1)',
      ].join('\n'),
    )
    await fs.chmod(fakeAivaultPath, 0o755)
    ...
    const syncBody = (await syncResponse.json()) as {
      ...
      errors: string[]
      syncState: { status: string; lastError?: string }
    }
    expect(syncBody).toEqual(
      expect.objectContaining({
        ok: false,
    ...
    expect(syncBody.errors).toHaveLength(1)
    expect(syncBody.errors[0]).toContain(
      'aivault could not read stored Plaid credentials for sync',
    )
    expect(syncBody.errors[0]).toContain('Original: serde error')
    expect(syncBody.syncState).toEqual(
      expect.objectContaining({
        status: 'error',
        lastError: syncBody.errors[0],
      }),
    )
    ...
    const calls = (await fs.readFile(fakeAivaultLogPath, 'utf8'))
      .trim()
      .split('\n')
      .map((line) => JSON.parse(line) as { capability: string })
    expect(calls).toEqual([{ capability: 'plaid/accounts-sync' }])

    const connections = JSON.parse(
      await fs.readFile(path.join(dataDir, 'connections.json'), 'utf8'),
    ) as Array<{ itemId: string; status: string; lastError?: string }>
    expect(connections).toEqual([
      expect.objectContaining({ itemId: 'item-alpha', status: 'connected' }),
      expect.objectContaining({ itemId: 'item-beta', status: 'connected' }),
    ])
    expect(connections.some((connection) => connection.lastError)).toBe(false)
  })
