...
payload returns **raw native-currency** `valueForSum`/`currentBalance` (e.g. rob's
CAD `PCA` checking is `36087` CAD), with **no** reporting/FX field. So a
client-side sum mixes CAD+USD 1:1 — e.g. raw sum `$227,064` vs the authoritative
FX-normalized `Accounts.Sum()` card `$242,381`. The formula layer already computes
...
> `GroupBy(liquidityClass)` previews as a table (Illiquid $549,161 / Liquid
> $47,226 for rob), card renders live with ƒ-inspect. Open follow-ups: Layer B
...
`liquidityClass = tier===undefined ? 'na' : tier<=2 ? 'liquid' : 'illiquid'`. This **exactly** satisfies rob's fixed mapping: cash (t0) + taxable brokerage (t2) = **liquid**; retirement + real estate (t3) = **illiquid**. The 4-tier label is upside for later (runway, waterfall) at no cost to the card.
...
Manual rows bypass all provider derivation — `buildManualAccount` passes `subtype`/`isAsset`/`isLiability` verbatim and `subtype` is free-form (`z.string().optional()`), so rob's home value arrives as `{type:'other', subtype:'real_estate', isAsset:true}` and his mortgage as `{type:'mortgage'}`. Because derivation lives at **read-time** in `normalizeFormulaData` (which runs for manual rows), both classify with zero extra input:
...
- **Layer A (v1, ship this):** the row-column override on the manual write path above. `normalizeAccountLiquidity` checks `normalizeLiquidityToken` first, so a stored `liquidity`/`liquidityTier` always beats the derived default. No registry, no merge — enough to ship the card and fix rob's home/mortgage.
...
**What rob saw:** the Review tab rendered **143 individual rule-warnings**
("Review large expense: Wise Inc WISE", repeated many times for the same merchant,
...
The valuable unit is "**Label all N 'Wise Inc WISE' transactions** → Transfer", not
...
the user's mental model is **"I taught it — new Wise charges should auto-be
...
`Wise Inc WISE` shows up as 18 "expenses" totaling **$173k** — that's clearly money
...
`money.transactions.reviewGroups` params. This catches real Wise-style transfer
...
which is the expected state for Rob's Wise rail until the Canadian destination
...
`candidateSearches`, so agents can resolve multi-row Wise rails one transfer at
...
USD-to-CAD Wise rail) without applying the source-currency amount filter, so FX
...
owned by the user but not linked yet, such as USD -> Wise -> Canadian bank. A
...
Canadian institutions such as Desjardins without disconnecting existing US
...
I also applied a user-confirmed local rule in the personal workspace:
`merchantId = wise-inc-wise`, `direction = expense` →
`moneyFlow.role = "transfer"` with `teachRule: true`. That labeled 18 existing
Wise transfer rows, refreshed metrics, removed Wise from largest expenses, and
cleared the money-flow review warning. Future matching Wise rows should
...
monthly costs. On Rob's live import this reduced the active subscription count
from 21 to 16 and monthly subscription cost from about `$902` to about `$830`.
...
I applied three high-confidence local merchant groups in the personal workspace:
`PwP Work Privacycom`, `Pittsburgh Water Utility`, and
`PwP Clerky Inc Privacycom`. The merchant review queue now has 0
`needs-group` items, and Pittsburgh Water collapsed from duplicate recurring
...
For Rob's current live import this correctly explains why mortgage/retirement
...
On Rob's current live import this turns all 9 empty cards into actionable
repair paths: 3 debt/APR cards request liabilities, 1 investment trend card
requests investments, and the sparse tax/shared-expense cards point at
proposal-first classification.
...
selected 3 transactions from 404 candidates, and produced 3 proposals.
...
Live measurement on Rob's current imported workspace after context reuse and
warehouse artifacts: projected `/api/cards` reads in about 7-8ms, projected
...
current live import. Frontend and agents should use the projected
