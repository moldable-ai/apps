...
> (need chips + teach copy), Review **Subscriptions** segment ($1,828/mo, Skip/Cancel),
...
- [x] List accounts per institution (name, type, mask, balance) — verified vs USAA
- [x] Add another account → inline `ConnectDialog`
- [x] Manual "Sync all" — **executed live & verified** (`POST /api/sync` →
      `ok:true`, 1 connection synced, 7 accounts / 649 txns, 12 metrics refreshed,
...
- [~] Disconnect an institution — wired w/ confirm (`DELETE /api/connections/:itemId`);
  **not live-verified** (won't disconnect the real USAA to test)
...
- [x] **Background ETL / processing UX** (per rob: "more of an ETL setup… make the
...
Per rob: simple at-a-glance roll-up cards + the ability to tap a rolled-up value
and drill into what made it up (a push/pop nav stack).

> **Architecture correction (per rob):** the first cut shipped these roll-ups as
> bespoke React components (`CreditCardsCard`, `AssetsLiquidityCard`) that summed
> `/api/accounts` client-side. rob caught it: _"there's no formula on their
...
- [x] **Credit card balances → the seeded `credit-utilization` card**
      ...
      Overview ("Credit Utilization", $797.58 total, per-card rows, ƒ inspect).
- [x] **Liquid vs illiquid assets → the seeded `liquid-vs-illiquid` card**
      ...
      **liquidity** once (a `liquidityTier` 0–3 + `liquidity` label + `liquidityClass`
      ...
      cards. This is the **true** answer to rob's "what money is actually
      available," and it **replaced the interim `assets-by-type` card** on
      **Overview** and **FIRE** (`personas.ts` swap + `PATCH /api/dashboards/:id`).
      **Verified live** on Overview: "Liquid vs Illiquid", $596,387 total →
      **Illiquid $549,161 (92%)** (real estate + IRA + RRSP) vs **Liquid $47,226
      (8%)** (checking + savings); ƒ flips to the `GroupBy(liquidityClass)` formula.
...
- [x] **Manual accounts** (`ManualAccountForm`, `useAddManualAccount` / update /
      ...
      a real account. Per rob's refinement, **creation/editing lives only on the
      Accounts page** ("Manual accounts" section) — the dashboard cards just
      reflect them (no creation UI on cards). **Verified E2E live**: a $282,000
      "1226 Love Street" real-estate asset rob entered himself shows in net worth
      _and_ as "Real Estate $282,000 (47%)" inside the formula-backed Assets by
      Type card — manual data feeding a real formula card, end to end. (Earlier
      curl round-trip also verified a −$400,000 mortgage liability: net worth
      dropped, shown red, "as of" preserved, then deleted/restored.)
- [x] **Drill-down nav stack** (`DrilldownDrawer`) — a right drawer with a level
      ...
      `/api/cards/:id/transactions`). **Verified live** (Groceries → 210
      transactions; Liquid → 3 accounts → PCA detail with back).
- [x] **New cards added to personas, not the codebase** (`personas.ts`): Overview
      ...
      `liquid-vs-illiquid`. Provisioned to rob's live dashboards via
      `PATCH /api/dashboards/:id` (+ a `POST /api/warehouse/rebuild` to materialize
      the card in his existing workspace, since `DEFAULT_CARD_DEFINITIONS` only
...
Redesigned per rob: **one scrollable view of all dashboards** with a sticky pill
...
clean line glyphs (the heavy `bg-muted` icon chips were removed per rob).

- [x] Single scroll of all dashboards (full masonry of cards each)
- [x] **Bento grid** (rebuilt twice per rob: free-form masonry always gapped
...
- [x] **Formula inspector is now a centered modal** (rebuilt per rob: "can't see
...
Reframed (after rob's "WARNING/OPPORTUNITY is cryptic, what do I do?" feedback)
...
- [ ] Dedicated proposal-approval queue (the Categorize cards already consume
  ...
  > **2026-06-04 Categorize redesign** (per rob: "much cleaner, not super text
...
**deliberately gated** behind rob's standing instruction not to trigger Plaid
SMS / mutate real financial data. They are not "unbuilt" — they await a per-
action go-ahead:

- Connect a new institution (Plaid Link → real SMS to rob)
- Sync all (`POST /api/sync` — refreshes the real USAA pull)
- Disconnect an institution (`DELETE /api/connections/:itemId` — destructive)
- Live label apply / agent classify pass (mutates rob's data / spends AI)
...
- opening the connect URL launches a real Plaid Link session against the live bank
  and **texts the user a real verification code**. Verify the `ConnectDialog` UI only
...
- [x] **"Living off savings" drawdown reframe** — when income is negligible (not
      ...
      burn ("Living off savings · $10,136 / mo"), pairing with Runway. Detected
