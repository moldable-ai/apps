...
const PLANT_CARE_SCHEMA: Record<string, unknown> = {
  type: 'object',
  properties: {
    summary: {
      type: 'string',
    ...
    light: { type: 'string', description: 'Light requirements.' },
    water: {
      type: 'object',
      properties: {
        intervalDays: {
          type: 'integer',
          description: 'Typical number of days between waterings.',
        },
  ...
  required: ['summary', 'water', 'careMarkdown'],
}
...
const PLANT_IMAGE_IDENTIFICATION_SCHEMA: Record<string, unknown> = {
  type: 'object',
  properties: {
    commonName: {
      type: 'string',
...
export async function identifyPlantImage(
  ...
  try {
    ...
    if (!result.commonName?.trim()) return undefined
    return result
  } catch (error) {
...
export async function generateCareProfile(
  ...
  try {
    ...
    const system =
      ...
      'Do not invent a watering interval if you are unsure — omit it instead.'
    ...
    care.generatedAt = new Date().toISOString()
    care.model = process.env.MOLDABLE_APP_ID
