...
function postToMoldable(message: Record<string, unknown>) {
...
export function DeckEditor({
  ...
  const applyLocalSlideUpdate = (nextDeck: Deck, slide: Slide) => {
  ...
  const commitTitle = async () => {
    ...
    await api.updateDeck(deck.id, { title: next })
    onChanged()
  }
...
function HistoryPanel({
  ...
  const doRevert = async (versionId: string) => {
    ...
    try {
      await api.revert(deck.id, versionId)
      onReverted()
    } catch (e) {
...
function AssetsPanel({
  ...
  onClose: () => void
  onChanged: () => void
  onSlideUpdated: (deck: Deck, slide: Slide) => void
}) {
  const template = getTemplate(deck.templateId)
  ...
  const handleUseImageOnSlide = async (fileName: string) => {
    ...
    try {
      ...
      onSlideUpdated(result.deck, result.slide)
    } catch (e) {
...
function PublishMenu({
  ...
  const published = deck.published
  ...
  // so updatedAt only moves past publishedAt when the content actually changes.)
  const dirty =
    !!published &&
    !!deck.updatedAt &&
  ...
  const toggle = async (next: boolean) => {
    if (busy) return
    if (next) {
      const res = await onPublish(deck.id)
          ...
          <div className="flex items-center justify-between gap-3">
            <div className="min-w-0">
              <div className="text-sm font-semibold">
                {published ? 'Published' : 'Publish deck'}
              </div>
              <div
                className={`text-xs ${dirty ? 'font-medium text-red-500' : 'text-muted-foreground'}`}
              >
                {published
                  ? dirty
            ...
            <Switch
              ...
              aria-label={published ? 'Unpublish deck' : 'Publish deck'}
            ...
            <>
              ...
              <div className="mt-2 flex items-center gap-1">
                ...
                <PanelAction
                  icon={
                    busy ? (
                      ...
                      <span className="relative inline-flex">
                        ...
                        {dirty ? (
                          <span className="ring-popover absolute -right-1 -top-1 size-1.5 rounded-full bg-red-500 ring-2" />
                        ) : null}
                      </span>
                    )
                  }
                  emphasis={dirty}
                  disabled={busy}
                  onClick={() => void onPublish(deck.id)}
                >
                  {dirty ? 'Republish to update' : 'Republish latest'}
                </PanelAction>
              </div>
            </>
          ) : !canPublish ? (
            <div className="text-muted-foreground mt-3 text-xs">
              Add a slide first to publish.
            </div>
          ) : null}
