...
export interface Template {
  ...
  tagline: string
  /** Picker filter chips, e.g. ['Fundraising','Strategy']. First entry is primary. */
  categories: string[]
  ...
  /** Letterbox color around the 16:9 stage. */
  stageBg: string
  ...
  assets?: string[]
  /** A full sample deck demonstrating the style's range. */
  sampleSlides: Slide[]
}
...
/** Derive the deck theme (fontLinks + css + stageBg) from a template. */
export function templateTheme(template: Template): DeckTheme {
  const tokens: Record<string, string> = {
    '--slide-bg': template.tokens['--bg'] ?? '#ffffff',
    ...template.tokens,
  }
  return {
    fontLinks: template.fonts.links,
    stageBg: template.stageBg ?? template.tokens['--bg'],
...
export function templateMeta(template: Template): TemplateMeta {
  return {
    ...
    slideCount: template.sampleSlides.length,
  }
}
