// AI image generation + editing for slide assets, via the host image proxy
// (gpt-image). Generated images are saved as deck assets and referenced from
// slide HTML/CSS with relative `assets/<file>` paths.
...
/** Generate an image from a prompt and save it as a deck asset. */
export async function generateSlideImage(
  ...
  await writeAsset(workspaceId, deckId, fileName, Buffer.from(b64, 'base64'))
  return { path: `assets/${fileName}`, fileName }
}

export interface EditImageInput {
  /** Existing deck asset to edit (filename or `assets/<file>`). */
  source: string
  prompt: string
  size?: string
  fileName?: string
...
/** Edit/tweak an existing deck image and save the result as a new asset. */
export async function editSlideImage(
  ...
  const baseImagePath = assetPath(workspaceId, deckId, sourceName)
